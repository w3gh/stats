...
class MostWinsHero extends Portlet
{
	...
	public function renderContent()
	{
		...
		$sql = "
			...
			WHERE LOWER(name) = LOWER(:username)
				AND(
						(winner=1	AND dp.newcolour>=1	AND dp.newcolour<=5)
					OR 
						(winner=2 AND dp.newcolour>=7 AND dp.newcolour<=11)
					)
			GROUP BY original
		...
		$data['count']=     $list['wins'];

		$data['heroimg'] = CHtml::image($heropath,$data['heroname']);

		$this->render('mostWinsHero',$data);
	}
}